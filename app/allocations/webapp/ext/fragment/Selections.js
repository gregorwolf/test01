        ...
        onOpenValueHelp:function(event){
			...
			if (!this._dialog) {
				this._dialog = Fragment.load({
					id: view.getId(),
					name: "allocations.ext.fragment.FieldValueHelp",
					controller: {
                        onValueHelpDialogClose:(event)=>{
                            const id = event.getParameter("selectedItem").getBindingContext().getProperty("ID");
                            const field = event.getParameter("selectedItem").getBindingContext().getProperty("field");
                            ...
                            const listBinding = this.editFlow.getView().byId(this.editFlow.getView().getId()+ "--fe::table::inputFields::LineItem::View-innerTable");
                            listBinding.getBinding("items").create({"field_ID":id});
                            MessageToast.show("Custom handler invoked.");
                            //Allocations(ID='117396f8-1aaa-40f6-bb70-500e30f24960',IsActiveEntity=false)/inputFields
                        }
                    }
